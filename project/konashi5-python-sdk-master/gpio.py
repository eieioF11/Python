...
KONASHI_ADDR = "58:8E:81:AB:0F:E8"
...
class KonashiGpio:
    ...
    def _btn_press_cb(self, sender, data):
        for i in range(8):
            if data[i]&0x10:
                if i==0 and data[i]&0x01:
                    self._has_press = True
    ...
    async def run(self):
        async with BleakClient(KONASHI_ADDR) as konashi:
            ...
            await konashi.start_notify(KONASHI_UUID_GPIO_INPUT, self._btn_press_cb)
            ...
            await konashi.write_gatt_char(KONASHI_UUID_CONFIG_CMD, bytearray([KONASHI_CFG_CMD_GPIO, 0x01,0x20, 0x11,0x10, 0x21,0x10, 0x31,0x10, 0x41,0x10]))
            ...
            while await konashi.is_connected():
                if self._has_press:
                    ...
                    cnt += 1
                    cnt %= 0x10
                    await konashi.write_gatt_char(KONASHI_UUID_CONTROL_CMD, bytearray([KONASHI_CTL_CMD_GPIO, 0x10|b0, 0x20|b1, 0x30|b2, 0x40|b3]))
...
loop.run_until_complete(kg.run())
