...
async def run():
    async with BleakClient(KONASHI_ADDR) as konashi:
        ...
        while await konashi.is_connected():
            # logging.debug("Command: SoftPWM control")
            # await konashi.write_gatt_char(KONASHI_UUID_CONTROL_CMD, bytearray([KONASHI_CTL_CMD_SOFTPWM, 0x00,0x00,0x00,0x00,0x00, 0x01,0x00,0x00,0x00,0x00, 0x02,0x00,0x00,0x00,0x00, 0x03,0x00,0x00,0x00,0x00]))
            # logging.debug("Command: HardPWM control")
            # await konashi.write_gatt_char(KONASHI_UUID_CONTROL_CMD, bytearray([KONASHI_CTL_CMD_HARDPWM, 0x00,0x00,0x00,0x00,0x00, 0x01,0x00,0x00,0x00,0x00, 0x02,0x00,0x00,0x00,0x00, 0x03,0x00,0x00,0x00,0x00]))
