...
async def run():
    async with BleakClient(KONASHI_ADDR) as konashi:
        ...
        # await konashi.write_gatt_char(KONASHI_UUID_CONFIG_CMD, bytearray([KONASHI_CFG_CMD_UART, 0x81, 0x00,0xc2,0x01,0x00]))
        logging.debug("Command: SPI config")
        await konashi.write_gatt_char(KONASHI_UUID_CONFIG_CMD, bytearray([KONASHI_CFG_CMD_SPI, 0x80, 0xf4,0x01,0x00,0x00]))
        ...
        while await konashi.is_connected():
            ...
            cnt += 1
            data_str = "Count: {}\n".format(cnt)
            await konashi.write_gatt_char(KONASHI_UUID_CONTROL_CMD, bytearray([KONASHI_CTL_CMD_SPI_DATA])+bytearray(data_str, "utf8"))
